\usepackage{adjustbox}
\usepackage{amsmath}
\usepackage{array}
\usepackage{authblk}
\usepackage{bbding}
\usepackage[autocite=superscript,sorting=none]{biblatex}
\usepackage{caption}
\usepackage{booktabs}
\usepackage{hyperref}
\usepackage[margin=0.5in]{geometry}
\usepackage{nopageno}
\usepackage{graphicx}
\usepackage{multicol}
\usepackage{multirow}
\usepackage{seqsplit}
\usepackage{tabularx}
\usepackage{units}

% Delimit paragraphs with newlines rather than spaces.
\setlength{\parindent}{0pt}
\setlength{\parskip}{\baselineskip}

% Use the same font in the main text as in the figures.
\usepackage{fontspec}
\setmainfont{LiberationSans}[
Path           = fonts/,
UprightFont    = *-Regular,
BoldFont       = *-Bold,
ItalicFont     = *-Italic,
BoldItalicFont = *-BoldItalic]
\newfontfamily{\dingbatsfont}{DejaVu Sans}

% Use DejaVu Sans for symbol characters (namely checkmarks) because 
% Liberation Sans doesn't have these characters..
\usepackage[Dingbats]{ucharclasses}
\setTransitionsForSymbols{\dingbatsfont}{}

% Use the same font in equations as in the main text
\usepackage{mathastext}
\usepackage{isomath}

% Double space the draft.
\usepackage{setspace}
\doublespacing

% Tell LaTeX where to find my figures.
\graphicspath{
 {figure_1/}
 {figure_2/}
 {figure_3/}
 {figure_4/}
}

% Tell LaTeX where to find my bibliography.
\addbibresource{curated_refs.bib}
\addbibresource{uncurated_refs.bib}

% Don't number sections.
\setcounter{secnumdepth}{0}

% Define the UCSF colors.
\usepackage{xcolor}
\definecolor{ucsfnavy}{RGB}{  0  32  73}
\definecolor{ucsfteal}{RGB}{ 24 163 172}
\definecolor{ucsfolive}{RGB}{144 189  49}
\definecolor{ucsfblue}{RGB}{ 23 140 203}
\definecolor{ucsforange}{RGB}{244 128  36}
\definecolor{ucsfpurple}{RGB}{113 111 178}
\definecolor{ucsfred}{RGB}{236  24  72}
\definecolor{ucsfyellow}{RGB}{255 221   0}
\definecolor{ucsfmagenta}{RGB}{174  67 125}
\definecolor{ucsfblack}{RGB}{  0   0   0}
\definecolor{ucsfdarkgrey}{RGB}{180 185 191}
\definecolor{ucsflightgrey}{RGB}{209 211 211}

% Define a bunch of useful aliases.
\newcommand\ligrnaF[2][]{ligRNA\textsuperscript{+#2}\textsubscript{#1}}
\newcommand\ligrnaB[2][]{ligRNA\textsuperscript{−#2}\textsubscript{#1}}
\newcommand\captitle[1]{\emph{#1}}
\newcommand\seq[1]{\seqsplit{\texttt{#1}}}
\newcommand\code[1]{\texttt{#1}}

\newcommand\latin[1]{\emph{#1}}
\newcommand\etal{\latin{et~al.}}
\newcommand\apo{\latin{apo}}
\newcommand\Apo{\latin{Apo}}
\newcommand\holo{\latin{holo}}
\newcommand\Holo{\latin{Holo}}
\newcommand\invitro{\latin{in vitro}}
\newcommand\Invitro{\latin{In vitro}}
\newcommand\invivo{\latin{in vivo}}
\newcommand\Invivo{\latin{In vivo}}
\newcommand\insilico{\latin{in silico}}
\newcommand\Insilico{\latin{In silico}}
\newcommand\ecoli{\latin{E.~coli}}
\newcommand\yeast{\latin{S.~cerevisiae}}

\newcommand\bp[1]{\unit[#1]{bp}}
\newcommand\evt[1]{\unit[#1]{evt}}
\newcommand\evts[1]{\unitfrac[#1]{evt}{s}}
\newcommand\g[1]{\unit[#1]{g}}
\newcommand\hour[1]{\unit[#1]{h}}
\newcommand\kcalmol[1]{\unitfrac[#1]{kcal}{mol}}
\newcommand\kV[1]{\unit[#1]{kV}}
\newcommand\liter[1]{\unit[#1]{liter}}
\newcommand\mgmL[1]{\unitfrac[#1]{mg}{mL}}
\newcommand\minute[1]{\unit[#1]{min}}
\newcommand\mL[1]{\unit[#1]{mL}}
\newcommand\mM[1]{\unit[#1]{mM}}
\newcommand\molar[1]{\unit[#1]{M}}
\newcommand\ms[1]{\unit[#1]{ms}}
\newcommand\nguL[1]{\unitfrac[#1]{ng}{μL}}
\newcommand\ngx[1]{\unit[#1]{ng}}
\newcommand\nt[1]{\unit[#1]{nt}}
\newcommand\nM[1]{\unit[#1]{nM}}
\newcommand\pguL[1]{\unitfrac[#1]{pg}{μL}}
\newcommand\px[1]{\unit[#1]{px}}
\newcommand\rpm[1]{\unit[#1]{rpm}}
\newcommand\ugmL[1]{\unitfrac[#1]{μg}{mL}}
\newcommand\uL[1]{\unit[#1]{μL}}
\newcommand\uM[1]{\unit[#1]{μM}}
\newcommand\UuL[1]{\unitfrac[#1]{U}{μL}}
\newcommand\Vcm[1]{\unitfrac[#1]{V}{cm}}

\newcommand\namedcite[1]{\citeauthor*{#1}\autocite{#1}}

% Commands to define and reference figures and tables.
\newcounter{mainfig}
\newcommand\reffig[1]{%
 Figure \ref{mainfig:#1}%
}
\newcommand\fig[1]{%
 \refstepcounter{mainfig}%
 \label{mainfig:#1}
 \subsection{\reffig{#1}}
}

\newcounter{maintab}
\newcommand\reftab[1]{%
 Table \ref{maintab:#1}%
}
\newcommand\tab[1]{%
 \refstepcounter{maintab}%
 \label{maintab:#1}
 \subsection{\reftab{#1}}
}

\newcounter{suppfig}
\newcommand\refsuppfig[1]{%
 Figure S\ref{suppfig:#1}%
}
\newcommand\suppfig[1]{%
 \refstepcounter{suppfig}%
 \label{suppfig:#1}
 \subsection{\refsuppfig{#1}}
}

\newcounter{supptab}
\newcommand\refsupptab[1]{%
 Table S\ref{supptab:#1}%
}
\newcommand\supptab[1]{%
 \refstepcounter{supptab}%
 \label{supptab:#1}
 \subsection{\refsupptab{#1}}
}

% Commands to format figures in tables in the proper amounts of space.
\newcommand\figonecol[1]{%
 \begin{center}%
  \includegraphics[width=3.46in]{#1}%
 \end{center}%
}
\newcommand\figtwocol[1]{%
 \makebox[\textwidth][c]{\includegraphics[width=7in]{#1}}%
}
\newcommand\fignatsize[2][]{%
 \makebox[\textwidth][c]{\includegraphics[#1]{#2}}%
}
\newcommand\tabonecol[1]{%
 {\singlespacing%
 \begin{adjustbox}{width=3.46in,center=\textwidth}%
   \input{#1}%
 \end{adjustbox}%
 }
}
\newcommand\tabtwocol[1]{%
 {\singlespacing%
 \begin{adjustbox}{width=7in,center=\textwidth}%
   \input{#1}%
 \end{adjustbox}%
 }
}
\newcommand\tabsideways[1]{%
 {\singlespacing%
 % Neat trick to fill to the end of the page:
 %   \dimexpr\pagegoal-\pagetotal-\baselineskip\relax
 % I'm not using it because it seems to let the section header float down from 
 % the top of the page.
 \begin{adjustbox}{angle=90,max size={\textwidth}{9in},center=\textwidth}%
   \input{#1}%
 \end{adjustbox}%
 }
}
\newcommand\tabnatsize[1]{%
 {\singlespacing%
   \begin{center}%
    \input{#1}%
   \end{center}%
 }
}

