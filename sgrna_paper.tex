...
%
% ^.
Alternatively, engineering the guide RNA (sgRNA) component of CRISPR/Cas9 allows individual genes to be regulated independently by encoding different response elements into each sgRNA. Two methods are based on the idea of using an aptamer to control whether the sgRNA's \nt{20} target sequence (more succinctly called the spacer) is blocked by a complementary sequence installed elsewhere in the sgRNA \autocite{liu2016,tang2017}. %KEW - There's another reference that I think is missing here. The Moon Lab at Washington St. Louis published 'Programmable control of bacterial gene expression with the combined CRISPR and antisense RNA system', which I think is relevant to this intro. %
Another method is based on the idea of using the sgRNA to recruit protein regulators that need to be stabilized by a ligand to avoid degradation \autocite{maji2017}.%KEW - The previous sentence could be more clear%
...
To control Cas9 activity, we sought to insert an aptamer into the sgRNA such that the aptamer's ligand-bound and ligand-free states yielded two alternative sgRNA structures: one permissive to Cas9 binding (and therefore activity), and the other restrictive. Which aptamer state is permissive to Cas9 binding determines the type of control provided (\reffig{1}a,b). 

We began by asking which sites in the sgRNA were most responsive to the insertion of a small-molecule aptamer, as well as which strategies for linking the aptamer to the sgRNA were most effective. We replaced three different stem loops of the sgRNA (\reffig{1}c) that had been previously shown be to solvent-exposed in the crystal structure of Cas9 ternary complex \autocite{nishimasu2014}, and exhibit various levels of tolerance to mutations \autocite{briner2014}. To test different linking strategies, we replaced different parts of each stem with the aptamer, splitting the sgRNA in half and using the aptamer to bring the halves together, and designing specific strand displacements for the \apo{} and \holo{} states (\reffig{1}d).  %KEW - the meaning of the last sentence here is a bit unclear and I'm unsure how to make a suggestion beyond changing the preposition.
Using an \invitro Cas9 cleavage assay to measure activity, we tested a starting set of 97 rationally designed sgRNAs. We used the theophylline aptamer due to its small size and high affinity for its ligand \autocite{jenison1994}.
%I think that theophylline should be mentioned farther up. Also, it would be worth noting that theophylline isn't naturally found in the cell.
We found functional sensors for all three insertion sites, with the the strand displacement strategy being the most successful (\reffig{1}e, \refsupptab{rational-designs}). We also observed a concentration-dependence on theophylline for Cas9 activity for our strongest designs (\reffig{1}f).
Unexpectedly, we found that inserting the aptamer into the nexus yields sensors that respond to the absence of theophylline, rather than to the presence of it.
%KEW - First, you didn't lay out what your expectations were to use 'unexpectedly'. Second, I think you should expand this part. The results here are already interesting and I think you're brushing over them. Maybe describe why the nexus was unexpected, and you don't even mention the other two insertion sites. My point is that after reading this, I'm not sure what insights you've gained although they are implied they are there.
Knowing how to design both ligand-activated and ligand-deactivated sgRNAs \invitro{}, we next sought to find sensors that would function robustly in \ecoli{}.
