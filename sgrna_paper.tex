...
%¶ Our strategy is to insert an aptamer into the guide RNA...
%  In this work, we pursued the strategy of...

    %§ Briefly review alternative sgRNA strategies (use the paragraph from the discussion, but remove the descriptions of the other methods, just reference them).

    %§ Succinctly list benefits of my approach.

The alternative is to focus on engineering Cas9's guide RNA (sgRNA).  This approach has also been explored, but not as thoroughly.  One pair of methods is based on the idea of using an aptamer to control whether the \nt{20} target sequence (more succinctly called the spacer) is blocked by a complementary sequence installed in the sgRNA \autocite{liu2016,tang2017}.  Another method is based on the idea of using the sgRNA to recruit protein regulators that need to be stabilized by a ligand to avoid degradation \autocite{maji2017}.  

Our goal in this study was to 

The alternative is to focus on engineering Cas9's guide RNA (sgRNA).  This approach has also been explored, but not as thoroughly.  One pair of methods is based on the idea of attaching a sequence to the sgRNA that can base pair with the \nt{20} target sequence (more succinctly called the spacer) and using an aptamer to control that base pairing \autocite{liu2016,tang2017}.  Another method is based on the idea of making protein regulators that are recruited by modified sgRNAs, but that need to be stabilized by a ligand to avoid being degraded \autocite{maji2017}.  

attaching a sequence to the sgRNA that can base pair with the \nt{20} target sequence (more succinctly called the spacer) and using an aptamer to control that base pairing 
...
This was the approach taken by \namedcite{liu2016}, who introduced a sequence on the 3' side of the sgRNA with complementarity to the \nt{20} target sequence (also called the spacer) and used an aptamer-based strand displacement strategy to dislodge it in the presence of ligand, and \namedcite{tang2017}, who introduced a sequence on the 5' side of the sgRNA with complementarity to the spacer and used a ligand-controlled hammerhead ribozyme to cleave it off upon the addition of ligand.

This was the approach taken by \namedcite{liu2016} and \namedcite{tang2017}, who both appended sequences to the sgRNA that were complementary to the \nt{20} target sequence (more succinctly called the spacer) and used ligand-controlled aptamers to control whether or not 

Both studies put the sgRNA under the control of a small-molecule ligand by 

This approach has also been explored, but not as thoroughly.  Two methods have been published based on the idea of appending a sequence to the sgRNA that is complementary to the \nt{20} target sequence (more succinctly called the spacer).  The first used an aptamer-based strand-displacement strategy to dislodge the complementary sequence \autocite{liu2016}, while the second used a ligand-controlled hammerhead ribozyme to cleave it off \autocite{tang2017}.  In contrast, our goal in this study was to 

This approach has also been explored, but not as thoroughly.  \namedcite{liu2016} attached a sequence to the sgRNA that blocks the \nt{20} target sequence (more succinctly called the spacer) by base pairing with it, and used an aptamer-based strand-displacement strategy to 

\namedcite{tang2017} attached a similar blocking sequence, but used a ligand-controlled hammerhead ribozyme to cleave it off.

was complementary to the \nt{20} target sequence (more succinctly called the spacer) and used an aptamer-based strand-displacement strategy to dislodge it. 
...
This approach has also been explored, but not as thoroughly.  Existing methods are based on the idea of attaching a sequence to the sgRNA that can base-pair with the \nt{20} target sequence (more succinctly called the spacer) and using a small-molecule aptamer to control that base pairing.
...
% This paragraphs ends with "despite", and the next paragraph start with it.  I should find a different word to use.
...
% I should end this paragraph by discussing the strengths of my system in bacteria, i.e. many interesting bacteria that don't have genetic controls, my system has very little genetic footprint, don't even necessarily need promoters, other systems don't work in bacteria.  That let's me end this paragraph, which starts out negatively, on a positive note.  I should also make the discussion of the negative results more succinct.  The supplemental figures will have captions if you want to know more.
